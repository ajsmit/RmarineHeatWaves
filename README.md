...
We can also load the 0.25 degree Reynolds [OISST data](http://www.ncdc.noaa.gov/thredds/oisst-catalog.html) and apply the function pixel by pixel for each year. The figure below shows the `detect()` function applied to these data, and then we plot the trend in the marine heat wave count per decade. We can clearly see how the Agulhas Current is experiencing marine heat waves more frequently in recent decades, and that there are two smaller areas where the frequency of marine heat waves is actually decreasing with time. Please read the package [vignette](https://github.com/ajsmit/RmarineHeatWaves/blob/master/vignettes/gridded-event-detection.Rmd) to see how this is done.
